...
def init_dataloader(opt, mode):
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_sampler=sampler,num_workers = 4)  #batch_sampler 每次返回一批索引的策略
    return dataloader
...
def train(opt, tr_dataloader, model, optim, lr_scheduler, val_dataloader=None, global_model=None,attention_model=None):
    ...
    for epoch in range(opt.epochs):
        ...
        for batch in tqdm(tr_iter):  #tqdm显示进度条
            ...
            target_inds = target_inds.expand(num_classes, 1).long().reshape(-1)
            # target_inds = torch.zeros(num_classes*1,num_classes).cuda().scatter_(1,target_inds.unsqueeze(1),1)
            target_inds = torch.full([num_classes*1,num_classes],0.025).cuda().scatter_(1,target_inds.unsqueeze(1),0.9)
...
def main():
    ...
    PATH = 'D:\lcb\code1\deepemd_pretrain_model\miniimagenet\\resnet12\max_acc.pth'
    model = init_protonet(options)
